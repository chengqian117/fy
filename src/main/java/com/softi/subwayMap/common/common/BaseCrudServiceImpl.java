...
@Setter
public class BaseCrudServiceImpl<M extends BaseMapper<T>,T>  implements  BaseCrudService<T>{
    ...
    public void addOne(T t) {
        int insertRows = this.baseMapper.insert(t);
        if(insertRows != 1){
            throw new RuntimeException("新增"+ (StringUtils.isEmpty(getMessageBlock())?t.getClass().toString():getMessageBlock()) +"失败");
        }
    }

    @Override
    public void deleteByIds(String objIds) {
        String[] ids = objIds.split(",");
        List<String> idList = Arrays.asList(ids);
        int deleteRows =this.baseMapper .deleteBatchIds(idList);
        if(deleteRows != idList.size()){
            throw new RuntimeException("删除"+ (StringUtils.isEmpty(getMessageBlock())?this.getClass().toString():getMessageBlock()) +"失败");
        }
    }

    @Override
    public void updateByEntity(T t) {
        int resultRows = this.baseMapper.updateById(t);
        if(resultRows != 1){
            throw new RuntimeException("更新"+(StringUtils.isEmpty(getMessageBlock())?t.getClass().toString():getMessageBlock())+"失败");
        }
    }

    @Override
    public Page<T> findPageByEntity(T t, QueryRequest request) {
        Page<T> page = new Page<>();
        //根据什么排序
        //封装分页
        page.setCurrent(request.getPageNum());
        page.setSize(request.getPageSize());
        EntityWrapper<T> queryWrapper=new EntityWrapper<>();
    ...
    public boolean insertOrUpdate(T entity) {
        if (null != entity) {
            ...
            if (null != tableInfo && com.baomidou.mybatisplus.toolkit.StringUtils.isNotEmpty(tableInfo.getKeyProperty())) {
