...
import com.softi.subwayMap.modules.data.dao.MonthDataMapper;
...
public class MonthDataServiceImpl extends ServiceImpl<MonthDataMapper, MonthData> implements MonthDataService {
    ...
    public int sumDayDataByUserAndDay(String userId, String date) {
        ...
        if(list.size()==0){
            ...
            if(i==1){
                ...
                EntityWrapper<DayData> entityWrapper2=new EntityWrapper<>();
                //查询条件 按照索引顺序来
                entityWrapper2.eq("user_id",userId);
                entityWrapper2.between("day",date+"-01",date+"-31");
                entityWrapper2.eq("version","1");

                EntityWrapper<DayData> entityWrapper3=new EntityWrapper<>();
                //查询条件 按照索引顺序来
    ...
    @Override
    public Page<MonthData> selectPageByDto(SecondDataDto data) {
        EntityWrapper<MonthData> entityWrapper=new EntityWrapper<>();
